...
export async function upsertVolumeTier(productId: string, formData: FormData) {
  const service = new VolumeDiscountTierService(
    new VolumeDiscountTierRepository(prisma),
  );
  ...
  if (errorMsg) {
    redirect(
      `/admin/products/${productId}/volume-tiers?error=${encodeURIComponent(errorMsg)}`,
    );
  }
